...
class CardStopOver extends Component {
  ...
  startChat = () => {
    ...
    //tripService.createMatch(users, startTime, endTime, location)
  }
          ...
          <div id="card-stop-over-container">
            ...
            {opened && (
              <Fragment>
                <div>
                  <b>Interests</b>
                  <p>{interests.reduce((acc,interest,i,array)=>acc + interest + (i<array.length-1)? ', ':'','')}</p>
                  <b>Languages</b>
                  <p>{languages.reduce((acc,languages,i,array)=>acc + languages + (i<array.length-1)? ', ':'','')}</p>
                </div>
